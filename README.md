...
每天早上 7:30（Asia/Shanghai）由外部定时器触发，自动抓取科技/消费新闻与 GitHub 热门项目，经**主编级三道闸门 + AI 重排**筛选后交由 DeepSeek 总结，渲染成响应式网页发布到 GitHub Pages、导出一份带双链的 Obsidian 笔记，并推送 HTML 邮件。

```
┌──────────────┐   ┌──────────────┐   ┌──────────────┐   ┌──────────────────┐   ┌──────────────┐
│ 1. 数据采集   │ → │ 2. 前置筛选   │ → │ 3. AI 提炼    │ → │ 4. 渲染           │ → │ 5. 通知/部署  │
│ 23 RSS      │   │ 硬否决/相关性 │   │ 主编重排      │   │ 瑞士极简网页       │   │ SMTP 邮件 +   │
│ GitHub API  │   │ 加权/强触发   │   │ + 只写不选    │   │ + Obsidian 双链笔记 │   │ gh-pages     │
│ HF JSON API │   │ (零成本)      │   │ DeepSeek ×3  │   │                   │   │              │
└──────────────┘   └──────────────┘   └──────────────┘   └──────────────────┘   └──────────────┘
```

## 工程评估准则

本项目里的每一次需求变更，都先过一遍下面这套评估流程，再动手写代码。
写成文档是为了让它成为固定动作，而不是看心情。**逐条都有本项目里的真实案例。**

### 1. 先验证前提，再讨论方案

**需求里最常见的问题不是方案不好，而是前提错了。** 前提错了，方案再精致也是白做。

动手前先问：这个现象的**证据**在哪？能不能自己复现或量测一遍？

> 案例：有一次需求说"信息源太泛导致硬核 AI 内容被漏掉，要换掉底层信息源"。
> 实测发现：原始素材池的跑题率只有 **3%**，几乎每个源都是干净的；
> 而最终发布的早报跑题率是 **38%**。噪声不是源带来的，是**筛选层挑出来的**。
> 如果按原方案换源，问题一条都解决不了。
>
> 同一个需求还说某条内容"被彻底漏掉"。查下来：那个源本来就在启用列表里，
> 而且**换源后依然会漏** —— 因为它在候选池里，是被"让模型自己挑 8 条"这一步
> 丢掉的。真正的修复点是选择机制，不是信息源。

### 2. 数字要自己量，不要接受转述

需求里的数字（比例、阈值、次数、时间）都要重新测一遍。测出来的数经常和直觉相反。

> 案例：需求里举的强触发词例子包含 `LLM`。实测 `AI` 一词能命中最终稿 **34%** 的条目 ——
> 用这种宽泛类别词做"强制入选"等于没有筛选。所以词表最终只放具名实体。

### 3. 不迎合，有异议就直接说，并给出依据和风险

发现问题要当场讲清楚"哪里不对、依据是什么、照做会有什么后果"，而不是先照做了再说。

> 案例：需求要求"删除所有泛商业、泛科技源"。依据实测数据，这是错的：
> 硬核 AI 新闻（华为昇腾、阶跃 StepAudio、GLM、vivo AgentOS）**恰恰来自**
> 被点名要删的那批源；而两个消费板块的源一旦删掉，五个板块里有两个会永远空着。
> 最终只停用了有实测证据的源（见下方信息源章节），其余保留并改为在筛选层治理。

### 4. 主动指出对方没考虑的变量、成本和偏差

需求方通常只看到主路径。要主动补上：成本、延迟、失败模式、副作用、边界情况。

> 案例：需求担心"让大模型对几百条新闻打分会导致 token 失控"。
> 量化下来：把候选池收到 25 条/板块、只送标题和截断摘要，
> 实测重排调用 **10,252 输入 token / 88 输出 token**，
> 相对新闻写作调用的 ~13k token 是很小的一笔。担心是合理的，但代价可控，
> 所以采用了"本地规则粗筛 + 一次轻量重排"的链路，而不是放弃语义判断。
>
> 反过来也补了对方没提的成本：接入候选源时实测发现 TLDR AI 的 feed
> 一个条目就是**一整期 newsletter 的标题拼接**（正文为空、链接指向期号页），
> 接入会直接污染数据；arXiv cs.AI 每天 **214 条**论文标题，会把候选池淹没。
> 这两个源最终没有接入，理由都写在 config.py 的注释里，避免以后重复劳动。

### 5. 结论要能复现，测试要能钉住

改完必须有可复现的验证：离线自检断言、真实链路跑一遍、上线后看日志。
**"我改好了"不算结论，有数字的验证才算。**

> 案例：本项目的 `selftest.py` 有 311 项离线断言，不联网、不需要密钥。
> 每次改动都跑，历史上有多次是靠它发现问题的（例如自检把测试假数据
> 写进了真实产物目录，后来专门加了一道产物目录快照比对来钉死这类回归）。

### 6. 别把"看起来在筛选"当成筛选

警惕那些**规则存在但实际不起作用**的设计。要验证规则真的拦住了东西。

> 案例：短英文关键词按子串匹配时，`ai` 会命中 `email` / `domain` / `captcha`，
> 导致英文源几乎全能过闸门 —— 闸门形同虚设。改成词边界匹配才真正生效。
> 同类问题还有：源级时间窗太窄会让某个源**静默**变成 0 条（配了等于没配），
> 所以现在所有"配置了却没有产出"的情况都会打 WARNING。
...
重构前是"抓 300 条 → 塞 80 条给模型 → 让模型自己挑 8 条/板块"。
问题在于**选择权和写作权混在同一次调用里**：模型一边写 60 字摘要一边排序，
排序质量必然让位于文字质量 —— 这就是硬核内容被热点噪音挤掉的机制性原因。
...
采集 247 条
   │
   ├─ ① 本地硬否决（零成本）      政治/军事/宏观/股市/司法/人事 → 丢
   ├─ ② 本地板块相关性闸门        与板块话题不符 → 丢（专治宏观财经塞进消费板块）
   ├─ ③ 本地加权评分 + 强触发白名单 → 排序、截到 25 条/板块
   │                              ↓ 剩 58 条
   ├─ ④ DeepSeek 主编重排        语义价值判定，每板块留 5 条（10k token）
   │                              ↓ 剩 20 条
   └─ ⑤ DeepSeek 新闻写作        只写不选：提示词明确"不需要增删或替换"
```
...
| --- | --- | --- |
| 原始素材 | 304 条 | 247 条 |
| 送进模型的候选 | 82 条（未筛选） | 58 条（已过三道闸门） |
| 最终新闻条数 | 24 条 | 20 条 |
| 跑题条目 | **9 条（38%）** | **0 条** |

### 三道闸门

**① 硬否决**（`CONTENT_VETO_KEYWORDS`）—— 命中即丢。覆盖政治选举、军事情报、
宏观货币、股市行情、司法诉讼、人事任免六类，中英文各一份。

> ⚠️ **中英文两份词表都是必需的。** 国际源全是英文，只写中文否决词的话
> `Startup sued over data practices`、`Oura's $2.2B IPO` 这类会大摇大摆走进早报。
> 这个坑是实测踩出来的。

**② 板块相关性闸门**（`SECTION_HINTS`）—— 条目必须命中所在板块的话题词。
这条专治"宏观财经被源的 category 带进消费板块"（原 `中新网财经` 命中率 0/5 的根因）。

> ⚠️ 两个实测教训：话题词必须**双语**（国际消费板块的源是英文站，
> 只写中文词会让这个板块永远空着）；短英文词的匹配必须用**词边界**
> （子串匹配时 `ai` 会命中 `email`，闸门形同虚设）。

**③ 强触发白名单**（`GEEK_FORCE_KEYWORDS`）—— 命中即**无条件入选**，
并豁免上面两道闸门，且不受池子截断影响。保证写在代码里
（`ranking.merge_llm_selection` 的保底名额），不指望提示词里的一句"请不要漏掉"。

> ⚠️ **词表刻意收窄，只放具名实体**（`jev` / `claude` / `gemini` / `qwen` /
> `sota` / `开源权重` / `智能体` …），不放 `AI` / `LLM` / `模型` 这类宽泛类别词 ——
> 实测 `AI` 能命中 34% 的条目，用它做强制入选等于没筛。
> `selftest.py` 里有一条断言专门盯着这件事，防止以后有人把泛化词加回去。

### 重排的成本

第三次 DeepSeek 调用。实测 **10,252 输入 token / 88 输出 token**，
相对新闻写作调用的 ~13k token 是很小的一笔。控制手段有三条：
每板块候选上限 25 条、只送标题与截断摘要、超预算时压摘要长度而不是丢条目。

重排**失败可降级**：退回纯本地排序（`ranking.fallback_selection`），流水线不中断。

### 本次的源变更（每条都有实测依据）

| 源 | 处置 | 依据 |
| --- | --- | --- |
| 中新网财经 | **停用** | 进入最终早报的 5 条**全部**不是消费新闻（美联储加息 / A股粮食 / 汽车全球化 / 扬州港风电 / 华龙一号核电）。命中率 0/5，根因是栏目定位是宏观财经却被标成了 consumer |
| Wired | **停用** | 入选的 3 条全是政治/监控类（中期选举 / 国防部 UAP / Flock 车牌摄像头），对科技产业板块 0 命中 |
| Fashion / Grocery / Food / Marketing Dive | **停用** | 四个源在最近一期合计贡献 0 条入选内容，国际消费板块改由 Retail Dive + Modern Retail 支撑 |
| Hacker News 高分 | **新增** | 24 小时内稳定 8–15 条，与已有的 HN 首页互补 |
| Lobsters | **新增** | 24 小时内 20+ 条，开发者社区高信噪比 |
| Simon Willison | **新增** | 量少（2 条/天）但信号极强，是 AI 圈的风向标 |
| HuggingFace Daily Papers | **新增** | JSON 接口，社区投票的每日热门论文 |
| HuggingFace Trending Models | **新增** | JSON 接口，按 trendingScore 排序的热门模型 |

**实测排除、没有接入的候选源**（避免以后重复劳动）：

| 候选源 | 为什么不接 |
| --- | --- |
| TLDR AI | 一个条目就是**一整期 newsletter 的标题拼接**（`Muse connectors, Meta SAM 3.1, Gemini hacks companies`），正文为空、链接指向期号页而非单条新闻，无法拆成单条资讯 |
| arXiv cs.AI | 每天 **214 条**论文标题，会把候选池淹没。论文维度已由 HuggingFace Daily Papers 覆盖，那是有社区筛选的 |
| Anthropic News | 该 RSS 不存在，返回空 feed |
| GitHub Blog / DeepMind Blog | 更新太慢，24 小时内 0 条 |

### 新增源的接入陷阱

**HuggingFace 的两个接口是 JSON API 不是 RSS**，塞进 `RSS_SOURCES` 会让
feedparser 直接解析失败，所以走了专用的抓取函数（`fetch_hf_streams`）。

**`daily_papers` 是滚动榜单，没有"过去 24 小时"的概念** —— 实测最新一篇也常是
两三天前提交的。用全局 24 小时窗口会把这个源**静默清空**（配了等于没配），
所以给了它独立的 `HF_LOOKBACK_HOURS`（默认 48）。

**热门模型榜反而不能按时间过滤** —— `modelId` 对应的仓库可能是去年建的，
按创建时间筛会把真正在爆的模型误杀。榜单本身就是"最近在涨"的信号。

### 源级时间窗

源配置支持 `lookback_hours` 字段覆盖全局窗口。日更/周更节奏的源
（newsletter 类）用全局 24 小时会被整个过滤掉，这时在源上单独放宽，
而不是为了迁就一个源去动全局窗口。
...
├── fetcher.py                   # 数据采集：RSS（并发）+ GitHub Search API + HuggingFace JSON
├── ranking.py                   # 主编级前置筛选：硬否决 / 板块相关性 / 加权评分 / 强触发白名单
├── summarizer.py                # DeepSeek 提炼、提示词、主编重排、防幻觉校验、降级
...
├── selftest.py                  # 离线自检（311 项，无需密钥/联网）
├── requirements.txt
...
| `RERANK_ENABLED` | `true` | 是否启用 DeepSeek 主编重排（关掉则退回纯本地排序） |
| `RERANK_POOL_PER_SECTION` | `25` | 每个板块送进重排的候选上限（压住它就是压住 token 成本） |
| `RERANK_KEEP_PER_SECTION` | `5` | 重排后每个板块保留几条（4 个新闻板块 × 5 = 20 条） |
| `RERANK_MAX_INPUT_CHARS` | `24000` | 重排调用的字符预算，超出后压摘要长度而非丢条目 |
| `FORCE_RESERVED_PER_SECTION` | `2` | 每个板块为强触发词预留的保底名额 |
| `HF_ENABLED` | `true` | 是否接入 HuggingFace 数据流 |
| `HF_PAPERS_LIMIT` | `25` | 每日热门论文抓取条数上限 |
| `HF_MODELS_LIMIT` | `15` | 热门模型抓取条数上限 |
| `HF_LOOKBACK_HOURS` | `48` | HF 论文的时间窗（它是滚动榜单，用 24 小时会被清空） |
...
10. **筛选与重排全程可降级**：本地三道闸门是纯函数、零网络；DeepSeek 重排失败时自动退回本地排序，
    流水线不中断。强触发白名单的保底名额在代码里，不依赖模型是否照做。
11. **空池必须报警**：相关性闸门配得过严时，某个板块会被静默清空（页面只显示"本时段无内容"，
    没有任何线索指向筛选规则）。所以任何板块候选池为空都会打 WARNING。
